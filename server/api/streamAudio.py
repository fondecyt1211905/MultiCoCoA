...
exchange_in = conf["sa_in_ex"]
type_exchange_in = conf["sa_in_ex_type"]
queue_in = conf["sa_in_q"]
exchange_out = conf["sa_out_ex"]
queue_out = conf["sa_out_q"]
...
def AudioService():
    ...
    def callback(ch, method, properties, data):
        ...
        print(os.path.join(os.getcwd(), "data", str(result.get("id_activity")), filename), flush=True)
        # Open File
        ...
        if os.path.exists(file):
            ...
            size_chunk = int(fps/conf["sa_chunk"])
            identification = helpers.menssage(
                ...
                channels = channels,
                chunk = size_chunk, 
                format_byte = conf["sa_format_byte"]
            )
